...
do
    ...
    do
	./view.py --architecture "$arch" --series $(cat /data/"$data"-list.txt | sed 's,^,/data/BERLIN-SERIES/epsg4326-shifted/,') --resnet-a resnet18 --model-state /data/output-$(echo $arch | sed 's,-segmenter,,')/super-duper-cloudy/"$arch"-None-best.pth --size 128 --stride 128 --batch-size 2 --output-dir /data/results/ --device cuda --num-heads 3 --name $(echo $arch | sed 's,-segmenter,,')-"$data" --cmyk
    done

    for arch in 18 34
    do
	./view.py --architecture attention-segmenter --series $(cat /data/"$data"-list.txt | sed 's,^,/data/BERLIN-SERIES/epsg4326-shifted/,') --resnet-architecture resnet"$arch" --model-state /data/output-resnet"$arch"/super-duper-cloudy/attention-segmenter-resnet"$arch"-best.pth --size 128 --stride 107 --batch-size 32 --output-dir /data/results/ --device cuda --num-heads 3 --name resnet"$arch"-"$data" --cmyk
    done
done
